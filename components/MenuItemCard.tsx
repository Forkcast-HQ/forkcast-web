...
export function MenuItemCard({
  ...
  // "rec" = uniform recommendation-rail card (fixed size, add button under photo,
  // clamped text) used by the dashboard and discover carousels.
  variant?: "default" | "rec";
}) {
  ...
  const dishHref = `/restaurant/${restaurantSlug}/dish/${item.id}`;
  const tierLabel = tier === "verified" ? "Verified" : tier === "published" ? "Published" : "Est. ±";
  const addTitle =
  ...
  // Uniform recommendation-rail card: fixed footprint, add button beneath the
  // photo, and text clamped to fixed line counts so every card is the same size.
  if (variant === "rec") {
    return (
      <article className="interactive-card group flex gap-3 rounded-2xl border border-black/[0.07] bg-white p-3">
        <div className="flex w-24 shrink-0 flex-col gap-2 sm:w-28">
          ...
          <button
            ...
            title={addTitle}
            className={cls(
              "inline-flex min-h-9 w-full items-center justify-center gap-1.5 rounded-xl px-2 py-2 text-sm font-semibold transition",
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <h3 className="truncate font-display text-[1.02rem] font-bold text-ink">
                <Link href={dishHref} className="hover:text-brand-700">
                  {item.name}
                </Link>
              </h3>
            </div>
          ...
          <p className="mt-0.5 line-clamp-2 min-h-[2.5rem] text-sm text-ink/55">{item.description}</p>

          <div className="mt-1.5 flex items-center gap-x-2.5 overflow-hidden text-xs">
            <span className="inline-flex shrink-0 items-center gap-1 font-semibold text-ink">
          ...
          <div className="mt-1.5 flex h-6 items-center gap-1.5 overflow-hidden">
            {tags.slice(0, 3).map((t) => (
              <span
                key={t}
                className="shrink-0 whitespace-nowrap rounded-full bg-brand-50 px-2 py-0.5 text-[11px] font-medium text-brand-700"
              >
                {TAG_LABEL[t]}
              </span>
            ))}
          ...
          <div className="mt-auto pt-2">
            {fit ? (
              ...
              <p className="truncate text-xs text-ink/40">Set up your profile for a Fit Score</p>
            )}
          </div>
        </div>
      </article>
